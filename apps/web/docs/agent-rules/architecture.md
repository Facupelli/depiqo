...
- Organize new feature work by business capability, not by technical concern or entity-only folders.
...
- `src/features/` holds feature slices, grouped by bounded context and business action.
...
- For backoffice SPA work, call the backend directly from feature API primitives; do not introduce TanStack Start server functions unless the feature explicitly requires server-side behavior.
...
- `src/features/` for feature-owned vertical slices
