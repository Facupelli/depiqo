...
describe(`GET ${path}`, () => {
  ...
  async function createRental(params: {
    ...
    await prisma.client.$executeRaw(Prisma.sql`
      INSERT INTO v2_rentals (
        ...
        ${params.periodStart}::timestamp, ${params.periodEnd}::timestamp,
        '2026-08-01 00:00:00'::timestamp, '2026-08-01 00:00:00'::timestamp
      )
    `);
  ...
  it('filters the inclusive local-date interval using a branch timezone override', async () => {
    ...
    const rentalId = await createRental({
      ...
      periodStart: '2026-08-09 23:30:00',
      periodEnd: '2026-08-10 01:00:00',
    });
  ...
  it('uses the tenant timezone when the branch has no override', async () => {
    ...
    const rentalId = await createRental({
      tenantId: tenant.id,
      branchId: branch.id,
      periodStart: '2026-08-10 02:30:00',
      periodEnd: '2026-08-10 04:00:00',
    });
    const client = await login(tenant.id);
  ...
  it('treats scalar UTC wall-clock values and tstzrange values as the same instant', async () => {
    const tenant = await fixtures.createTenant();
    ...
    const rows = await prisma.client.$transaction(async (tx) => {
      ...
      >(Prisma.sql`
        SELECT
          (r.period_start AT TIME ZONE 'UTC') = lower(tstzrange('2026-08-10T02:30:00Z', '2026-08-10T04:00:00Z', '[)')) AS "startsAtSameInstant",
          (r.period_end AT TIME ZONE 'UTC') = upper(tstzrange('2026-08-10T02:30:00Z', '2026-08-10T04:00:00Z', '[)')) AS "endsAtSameInstant",
          ((r.period_start AT TIME ZONE 'UTC') AT TIME ZONE 'America/Argentina/Buenos_Aires')::date = '2026-08-09'::date AS "localDateIsSessionIndependent"
        FROM v2_rentals r
        WHERE r.id = ${rentalId}
      `);
    });
