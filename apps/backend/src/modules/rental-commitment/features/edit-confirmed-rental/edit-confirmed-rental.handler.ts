...
> {
  ...
  async execute(command: EditConfirmedRentalCommand): Promise<EditConfirmedRentalResult> {
    ...
    if (branchFacts.isErr())
      return err(this.toApplicationError(new BranchUnavailableForRentalError(branchId), context));
    ...
    if (isCatalogSelectionError(error)) {
      switch (error.code) {
        ...
        case 'RentalOfferNotFound':
          return editConfirmedRentalError('rental_commitment.rental_offer_not_found', error.message, error, context);
