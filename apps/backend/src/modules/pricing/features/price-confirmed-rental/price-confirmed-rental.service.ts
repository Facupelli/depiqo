...
export class PriceConfirmedRentalService {
  ...
  async priceConfirmedRental(input: PriceConfirmedRentalInput): Promise<Result<RentalPriceSnapshotV1, PricingError>> {
  ...
  private validateInput(input: PriceConfirmedRentalInput): PricingError | null {
    ...
    if (!input.customerId?.trim()) {
      return new InvalidPricingInputError('customerId is required for confirmed rental pricing.');
    }
  ...
  private validateHydratedRatePlans(
    selections: Array<
      PriceConfirmedRentalInput['selections'][number] & { ratePlan?: PricingInput['selections'][number]['ratePlan'] }
    >,
  ): PricingError | null {
