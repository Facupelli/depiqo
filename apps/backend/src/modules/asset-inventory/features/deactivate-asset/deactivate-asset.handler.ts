...
import { DeactivateAssetError, deactivateAssetError } from './deactivate-asset.errors';

export type DeactivateAssetResult = Result<void, DeactivateAssetError>;

@CommandHandler(DeactivateAssetCommand)
export class DeactivateAssetHandler implements ICommandHandler<DeactivateAssetCommand, DeactivateAssetResult> {
  constructor(
  ...
  async execute(command: DeactivateAssetCommand): Promise<DeactivateAssetResult> {
    ...
    if (!asset) {
      return err(
        deactivateAssetError('asset_inventory.asset_not_found', 'Asset not found.', undefined, {
          assetId: command.assetId,
        }),
      );
    }

    const result = asset.deactivate();
    if (result.isErr()) {
      return err(
        deactivateAssetError('asset_inventory.invalid_asset_lifecycle_transition', result.error.message, result.error, {
