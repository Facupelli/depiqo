...
interface CreateBranchProblemDefinition {
  type: string;
  title: string;
  status: number;
  detail: string;
...
const CreateBranchProblemCatalog: Record<TenantManagementApplicationErrorCode, CreateBranchProblemDefinition> = {
...
export function toCreateBranchProblem(error: TenantManagementApplicationError): V2ProblemException {
  const definition = CreateBranchProblemCatalog[error.code];
