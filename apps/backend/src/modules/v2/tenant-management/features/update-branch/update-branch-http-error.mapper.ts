...
interface UpdateBranchProblemDefinition {
  type: string;
  title: string;
  status: number;
  detail: string;
...
const UpdateBranchProblemCatalog: Record<TenantManagementApplicationErrorCode, UpdateBranchProblemDefinition> = {
...
export function toUpdateBranchProblem(error: TenantManagementApplicationError): V2ProblemException {
  const definition = UpdateBranchProblemCatalog[error.code];
