...
interface GetCustomerProfileDetailProblemDefinition {
  type: string;
  title: string;
  status: number;
  detail: string;
...
const GetCustomerProfileDetailProblemCatalog: Record<
  TenantManagementApplicationErrorCode,
  GetCustomerProfileDetailProblemDefinition
> = {
...
export function toGetCustomerProfileDetailProblem(error: TenantManagementApplicationError): V2ProblemException {
  const definition = GetCustomerProfileDetailProblemCatalog[error.code];
