...
interface GetCurrentRentalCustomerProfileProblemDefinition {
  type: string;
  title: string;
  status: number;
  detail: string;
...
const GetCurrentRentalCustomerProfileProblemCatalog: Record<
  TenantManagementApplicationErrorCode,
  GetCurrentRentalCustomerProfileProblemDefinition
> = {
...
export function toGetCurrentRentalCustomerProfileProblem(error: TenantManagementApplicationError): V2ProblemException {
  const definition = GetCurrentRentalCustomerProfileProblemCatalog[error.code];
