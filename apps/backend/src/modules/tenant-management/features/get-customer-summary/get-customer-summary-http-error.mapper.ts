...
interface GetCustomerSummaryProblemDefinition {
  type: string;
  title: string;
  status: number;
  detail: string;
...
const GetCustomerSummaryProblemCatalog: Record<
  TenantManagementApplicationErrorCode,
  GetCustomerSummaryProblemDefinition
> = {
...
export function toGetCustomerSummaryProblem(error: TenantManagementApplicationError): V2ProblemException {
  const definition = GetCustomerSummaryProblemCatalog[error.code];
