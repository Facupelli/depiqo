...
import { useDeliveryAddressSuggestions } from "../delivery-address-suggestions.queries";

type DeliveryAddressAutocompleteProps = {
	id: string;
	name: string;
	value: string;
	isInvalid: boolean;
...
export function DeliveryAddressAutocomplete({
	...
	const query = useDeliveryAddressSuggestions(
		debouncedCanSearch ? debouncedValue : "",
	);
	const suggestions = query.data?.suggestions ?? [];
	const currentActiveIndex =
		activeIndex < suggestions.length ? activeIndex : -1;
		...
		>
			<Input
				...
				onKeyDown={(event) => {
					...
					if (event.key === "Enter" && currentActiveIndex >= 0) {
						event.preventDefault();
						const suggestion = suggestions[currentActiveIndex];
